...
- **A navigated preview, not a wall.** The ingot renders the document as bounded sections with an outline rail (scroll-spy), per-section copy, a molten scroll-driven level rail, and a raw toggle — and when **Chunking** is on, the preview switches to chunk blocks with stamped dividers (number · tokens · heading), a chunk rail with keyboard navigation, and per-chunk copy/download.
- **Chunk for RAG.** Any converted document splits into LLM-ready chunks — 256/512/1024-token presets (or a custom size), counted with the real gpt-tokenizer (cl100k_base), heading-aware boundaries, ~10% overlap (adjustable, with honest guidance), code fences and tables never split — downloadable as numbered `.md` files plus an index and a `chunks.json` metadata sidecar (heading paths, parent refs, table flags). Docs over 1 MB chunk in a one-shot worker.
